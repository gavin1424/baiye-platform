...
test("P30 正式 v1.0 Legal Review 已核准", async () => { const db = new D1(); const result = await applied(db); assert.equal(result.data.contract.legal_review_status, "approved"); assert.equal(result.data.contract.signing_available, true); assert.equal(result.data.contract.version, "v1.0"); });
