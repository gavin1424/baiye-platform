...
test("UI05 coupon notification is removed", () => assert.doesNotMatch(source, /NT\$100 迎新禮券已放入您的會員帳戶/));
...
test("UI12 signed refresh offers download and dashboard only", () => {
  assert.doesNotMatch(source, /查看已簽 PDF/);
  assert.doesNotMatch(source, />下載 PDF</);
  assert.match(source, /下載契約檔案/);
  assert.match(source, /返回承攬夥伴中心/);
  assert.doesNotMatch(source, /to=\{`\/partner\/contracts\/\$\{contract\.signature\.signature_id\}\/view`\}/);
});
test("UI13 coupon issue warning is removed", () => assert.doesNotMatch(source,/契約已簽署成功，但迎新禮券建立暫時失敗/));
