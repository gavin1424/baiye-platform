...
test("test provider generates an explicitly non-production invoice only in test", async () => {
  const adapter = getInvoiceProviderAdapter("mock_for_automated_test_only", { NODE_ENV: "test", INVOICE_TEST_MODE: "isolated" });
  const result = await adapter.issueInvoice({ request_id: "request-000001" });
  assert.match(result.invoice_number, /^TEST-INV-/);
  const allowance = await adapter.issueAllowance({ invoice_id: "invoice-1", amount_minor: 10000 });
  assert.equal(allowance.ok, true);
  assert.match(allowance.allowance_number, /^TEST-ALW-/);
  assert.equal((await adapter.voidInvoice({ invoice_id: "invoice-1" })).ok, true);
});
...
test("invoice migration includes requests, immutable document records and retry state", () => {
  ...
  assert.match(immutability, /immutable/);
  assert.match(immutability, /BEFORE DELETE ON invoices/);
});
