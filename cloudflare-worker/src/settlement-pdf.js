...
const money = (minor) => `NT$${Math.trunc(Number(minor || 0) / 100).toLocaleString("zh-TW")}`;
...
function stream(lines, page, pages) {
  ...
  commands.push("ET", `BT /F1 8 Tf 230 25 Td <${textHex(`創百業智慧鏈｜月結對帳單 ${page}/${pages}`)}> Tj ET`);
  return commands.join("\n");
}

export async function createSettlementPdf(input) {
  const statementHash = input.statement_hash || await sha256(JSON.stringify({
    statement_no: input.statement_no, merchant_id: input.merchant_id, period_start: input.period_start,
    period_end: input.period_end, calculation_version: input.calculation_version, rules_snapshot_json: input.rules_snapshot_json,
  ...
  const lines = [
    ...
    `代收訂金：${money(input.deposit_collected_minor)}`,
    `實際／估算金流手續費：${money(input.processing_fee_minor)}`,
  ...
  add("<< /Type /Font /Subtype /CIDFontType0 /BaseFont /MSung-Light /CIDSystemInfo << /Registry (Adobe) /Ordering (CNS1) /Supplement 7 >> >>");
  const refs = [];
  pageLines.forEach((page, index) => {
    const contentStream = stream(page, index + 1, pageLines.length);
    const content = add(`<< /Length ${encoder.encode(contentStream).length} >>\nstream\n${contentStream}\nendstream`);
    const object = add(`<< /Type /Page /Parent ${pagesObject} 0 R /MediaBox [0 0 595 842] /Resources << /Font << /F1 ${font} 0 R >> >> /Contents ${content} 0 R >>`);
    refs.push(`${object} 0 R`);
  });
  objects[pagesObject - 1] = `<< /Type /Pages /Kids [${refs.join(" ")}] /Count ${refs.length} >>`;
  let pdf = "%PDF-1.7\n%\xE2\xE3\xCF\xD3\n";
  const offsets = [0];
...
export function settlementCsv(statement, items = [], adjustments = []) {
  const rows = [
    ["對帳單編號", statement.statement_no], ["商家", statement.merchant_name || statement.merchant_id],
    ["期間", `${statement.period_start}~${statement.period_end}`], ["狀態", statement.status],
    ["訂單總額(分)", statement.total_order_amount_minor], ["代收訂金(分)", statement.deposit_collected_minor],
    ["金流手續費(分)", statement.processing_fee_minor], ["平台作業服務費(分)", statement.platform_service_fee_minor],
    ["稅務預留(分)", statement.tax_reserve_minor], ["扣繳(分)", statement.withholding_minor],
    ["調整(分)", statement.adjustments_minor], ["應撥店家(分)", statement.merchant_payable_minor], [],
    ["類型","來源","金額(分)","發生時間"],
    ...items.map((item) => [item.item_type,item.source_id,item.amount_minor,item.occurred_at]),
    ...adjustments.map((item) => [item.adjustment_type,item.source_reference || item.id,item.amount_minor,item.created_at]),
  ];
  const escape = (value) => `"${String(value ?? "").replaceAll('"','""')}"`;
  return new Uint8Array([0xef,0xbb,0xbf,...encoder.encode(rows.map((row) => row.map(escape).join(",")).join("\r\n"))]);
}
