...
export async function inventorySummary(db, merchantId) {
  const row = await db.prepare(`SELECT COUNT(*) total,
    ...
    FROM merchant_inventory_items WHERE merchant_id=?`).bind(merchantId).first();
...
export async function handleMerchantInventory(request, env, url, cors, authorization) {
  ...
  if (url.pathname === "/api/merchant-admin/inventory" && request.method === "GET") {
    const [items, menuItems, summary] = await Promise.all([
      db.prepare(`${inventorySelect} WHERE i.merchant_id=? ORDER BY m.sort_order,m.name`).bind(merchantId).all(),
      db.prepare(`SELECT m.id,m.name,m.status,CASE WHEN i.id IS NULL THEN 0 ELSE 1 END inventory_exists,
        i.stock_on_hand,i.inventory_enabled FROM merchant_menu_items m LEFT JOIN merchant_inventory_items i ON i.merchant_id=m.merchant_id AND i.menu_item_id=m.id
  ...
  if (url.pathname === "/api/merchant-admin/inventory" && request.method === "POST") {
    ...
    const existing = await db.prepare("SELECT id FROM merchant_inventory_items WHERE merchant_id=? AND menu_item_id=?").bind(merchantId, menuItemId).first();
  ...
  if (itemMatch && request.method === "PATCH") {
    const current = await db.prepare("SELECT * FROM merchant_inventory_items WHERE merchant_id=? AND id=?").bind(merchantId, clean(itemMatch[1], 160)).first();
  ...
  if (actionMatch && request.method === "POST") {
    const current = await db.prepare("SELECT * FROM merchant_inventory_items WHERE merchant_id=? AND id=?").bind(merchantId, clean(actionMatch[1], 160)).first();
    if (!current) return json({ error: "找不到此庫存項目。" }, 404, cors);
    ...
    try {
      await db.batch([
        db.prepare("UPDATE merchant_inventory_items SET stock_on_hand=stock_on_hand+?,updated_at=CURRENT_TIMESTAMP WHERE merchant_id=? AND id=?").bind(delta, merchantId, current.id),
        db.prepare(`INSERT INTO merchant_inventory_movements(id,merchant_id,inventory_item_id,menu_item_id,movement_type,quantity_delta,quantity_before,quantity_after,reason,actor_type,actor_id)
          SELECT ?,merchant_id,id,menu_item_id,?, ?,stock_on_hand-?,stock_on_hand,?,'merchant',? FROM merchant_inventory_items WHERE merchant_id=? AND id=?`).bind(uid("invmove"), type, delta, delta, reason || (type === "RESTOCK" ? "商家補貨" : null), actorId, merchantId, current.id),
      ]);
  ...
  if (movementsMatch && request.method === "GET") {
    const current = await db.prepare("SELECT id FROM merchant_inventory_items WHERE merchant_id=? AND id=?").bind(merchantId, clean(movementsMatch[1], 160)).first();
    if (!current) return json({ error: "找不到此庫存項目。" }, 404, cors);
...
export function deductionStatements(db, merchantId, orderId, lines, actorId) {
  return lines.flatMap((line) => [
    db.prepare(`INSERT INTO merchant_inventory_movements(id,merchant_id,inventory_item_id,menu_item_id,order_id,order_item_id,movement_type,quantity_delta,quantity_before,quantity_after,reason,actor_type,actor_id)
      ...
      FROM merchant_inventory_items i WHERE i.merchant_id=? AND i.menu_item_id=? AND i.inventory_enabled=1`).bind(uid("invmove"), orderId, line.order_item_id, line.quantity, line.quantity, actorId, merchantId, line.menu_item_id),
    db.prepare(`UPDATE merchant_inventory_items SET stock_on_hand=stock_on_hand-?,updated_at=CURRENT_TIMESTAMP WHERE merchant_id=? AND menu_item_id=? AND inventory_enabled=1`).bind(line.quantity, merchantId, line.menu_item_id),
  ]);
}
...
export function restoreStatements(db, merchantId, orderId, lines, actorType, actorId, reason) {
  return lines.flatMap((line) => [
    db.prepare(`UPDATE merchant_inventory_items SET stock_on_hand=stock_on_hand+?,updated_at=CURRENT_TIMESTAMP
      WHERE merchant_id=? AND menu_item_id=? AND EXISTS(SELECT 1 FROM merchant_inventory_movements d WHERE d.order_item_id=? AND d.movement_type='ORDER_DEDUCTION')
    ...
    db.prepare(`INSERT OR IGNORE INTO merchant_inventory_movements(id,merchant_id,inventory_item_id,menu_item_id,order_id,order_item_id,movement_type,quantity_delta,quantity_before,quantity_after,reason,actor_type,actor_id)
      ...
      FROM merchant_inventory_items i WHERE i.merchant_id=? AND i.menu_item_id=?
