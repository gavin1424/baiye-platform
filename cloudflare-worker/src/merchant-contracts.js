...
async function merchantContractContext(db, session, env) {
  ...
  if (!contract) {
    const latest = await db.prepare("SELECT * FROM merchant_contract_versions WHERE id IN ('merchant_service_v1_2_18000_addons','merchant_softpos_v1_0_24000') ORDER BY effective_date DESC,created_at DESC LIMIT 1").first();
