...
export async function handlePartnerRequest(request, env, url, cors, adminAuthorized = false) {
  ...
  if (path === "/api/partner/contract/current" && request.method === "GET") {
    ...
    return json(contract ? { ...contract, partner_legal_name: partner.legal_name, identity_completion_required: Boolean(partner.identity_completion_required), id_number_masked: partner.id_number_last4 ? `******${partner.id_number_last4}` : null, period_preview: period } : null, 200, cors);
  }
  ...
  if (path === "/api/partner/contract/sign" && request.method === "POST") {
    try {
      ...
      const result = { ok: true, signature_id: signatureId, public_id: publicId, pdf_hash: agreement.pdfHash, document_hash: agreement.documentHash, contract_period: contractPeriod, membership: { member_id: membership.member.id, member_no: membership.member.member_no, created: membership.created }, member_session: membership.session, welcome: membership.welcome };
      await completeContractOperation(db, operation.operation.id, result);
      return json(result, 201, cors);
    } catch (error) { return contractFailure(error, cors); }
  }
