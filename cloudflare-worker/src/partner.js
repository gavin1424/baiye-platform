...
export async function handlePartnerRequest(request, env, url, cors, adminAuthorized = false) {
  ...
  if (path === "/api/partner/apply" && request.method === "POST") {
    ...
    try {
      ...
      const member = await finalizePlatformMembershipBatch(db, membership);
      const contract = await activeContract(db);
      return json({ id: partnerId, partner_code: partnerCode, status: "pending_contract", state: "pending_activation", approved_at: approvedAt, approval_mode: "automatic", activation_url: activationUrl(invite.raw, env), activation_expires_at: invite.expiresAt, membership: member.member, welcome: { show: member.created, title: "歡迎成為創百業會員！", message: "您也已成為創百業會員，會員經營功能已連結。" }, coupon: null, contract: { version: contract?.version || null, legal_review_status: contract?.legal_review_status || "pending_review", signing_available: Boolean(contract?.legal_review_status === "approved" && contract?.approved_content_hash === contract?.content_hash) } }, 201, { ...cors, "cache-control": "no-store" });
    } catch (error) {
