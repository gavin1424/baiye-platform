import { authenticatePlatformMember, findPlatformMemberByPhone, normalizeTaiwanMobile } from "./platform-membership.js";
...
import {
  ADVISOR_COMMISSION_MODE, ADVISOR_COMMISSION_POLICY_VERSION, advisorCommissionSnapshot,
...
export async function createAdvisorBooking(db, request, input, member, env) {
  ...
  const terms = await db.prepare("SELECT * FROM advisor_consumer_terms_versions WHERE is_active=1 ORDER BY created_at DESC LIMIT 1").first();
  ...
  const result = await db.prepare(`INSERT INTO advisor_bookings(id,booking_code,advisor_id,service_id,platform_member_id,start_at,end_at,blocked_start_at,blocked_end_at,service_mode,status,service_price_minor,payment_status,payment_provider,test_data,customer_note,terms_version,terms_hash)
...
export async function recordAdvisorCommission(db, bookingId, { providerFeeMinor = 0, providerFeePolicy = "platform_absorbs", idempotencyKey = `commission:${bookingId}` } = {}) {
  const booking = await db.prepare("SELECT * FROM advisor_bookings WHERE id=?").bind(bookingId).first();
  if (!booking || booking.status !== "COMPLETED" || booking.payment_status !== "PAID" || Number(booking.test_data) === 1) return { created: false, excluded: true };
  const month = booking.completed_at.slice(0, 7), count = await db.prepare("SELECT COUNT(*) count FROM advisor_bookings WHERE advisor_id=? AND status='COMPLETED' AND payment_status='PAID' AND test_data=0 AND substr(completed_at,1,7)=?").bind(booking.advisor_id, month).first();
  const snapshot = advisorCommissionSnapshot({ servicePriceMinor: Number(booking.service_price_minor), providerFeeMinor, monthlyCompletedCount: Number(count.count), providerFeePolicy });
  const id = uid("advledger");
  await db.prepare(`INSERT OR IGNORE INTO advisor_commission_ledger(id,advisor_id,booking_id,entry_type,service_price_minor,provider_fee_minor,advisor_rate_bp,platform_rate_bp,commission_policy_version,monthly_completed_count_at_snapshot,advisor_share_minor,platform_share_minor,refund_policy_version,idempotency_key) VALUES(?,?,?,'COMMISSION',?,?,?,?,?,?,?,?,?,?)`)
    .bind(id,booking.advisor_id,booking.id,snapshot.servicePriceMinor,snapshot.providerFeeMinor,snapshot.advisorRateBp,snapshot.platformRateBp,snapshot.commissionPolicyVersion,snapshot.monthlyCompletedCountAtSnapshot,snapshot.advisorShareMinor,snapshot.platformShareMinor,snapshot.refundPolicyVersion,idempotencyKey).run();
  return { created: true, snapshot, id };
}

export async function reverseAdvisorCommission(db, bookingId, idempotencyKey) {
  ...
  const id = uid("advledger");
  await db.prepare(`INSERT OR IGNORE INTO advisor_commission_ledger(id,advisor_id,booking_id,entry_type,original_entry_id,service_price_minor,provider_fee_minor,advisor_rate_bp,platform_rate_bp,commission_policy_version,monthly_completed_count_at_snapshot,advisor_share_minor,platform_share_minor,refund_policy_version,idempotency_key) VALUES(?,?,?,'REFUND_REVERSAL',?,?,?,?,?,?,?,?,?,?,?)`)
    .bind(id,original.advisor_id,original.booking_id,original.id,-Number(original.service_price_minor),-Number(original.provider_fee_minor),Number(original.advisor_rate_bp),Number(original.platform_rate_bp),original.commission_policy_version,Number(original.monthly_completed_count_at_snapshot),-Number(original.advisor_share_minor),-Number(original.platform_share_minor),original.refund_policy_version,idempotencyKey).run();
...
export async function createAdvisorSettlementDraft(db, advisorId, periodStart, periodEnd) {
  ...
  await db.prepare("INSERT INTO advisor_settlement_statements(id,statement_no,advisor_id,period_start,period_end,gross_service_minor,advisor_share_minor,reversal_minor,net_payable_minor,status,provider_fee_policy,tax_withholding_mode,payout_block_reason) VALUES(?,?,?,?,?,?,?,?,?,'draft',?,?,?)")
    .bind(statementId,`ADV-STG-${Date.now()}`,advisorId,periodStart,periodEnd,gross,advisorShare,reversal,advisorShare,policy?.provider_fee_policy || 'policy_required',policy?.tax_withholding_mode || 'manual_review','PAYOUT_BLOCKED_POLICY_REQUIRED').run();
...
async function signAdvisorContract(request, env, cors, session) {
  const input = await request.json(), contract = await env.FINANCE_DB.prepare("SELECT * FROM advisor_contract_versions WHERE is_active=1 ORDER BY created_at DESC LIMIT 1").first();
...
export async function handleAdvisorPublic(request, env, url, cors) {
  ...
  if(profileMatch&&request.method==='GET'){
    ...
    return json({advisor:publicAdvisor(advisor),services:services.results||[],reviews:reviews.results||[],media:media.results||[],ai_faq:{scope:["老師介紹","服務項目","價格","預約方式","服務方式","公開 FAQ"],disallowed:["命理判斷","醫療、法律或投資指示"]},disclaimer:"本平台顧問服務主要屬於文化、娛樂、生活探索、個人成長與一般陪談用途，不替代醫療、心理治療、法律、投資或其他依法須由專業資格人員提供的服務。"},200,cors);
  }
...
export async function handleAdvisorPrivate(request,env,url,cors){
  ...
  if(url.pathname==='/api/advisor/customers'&&request.method==='GET'){const rows=await db.prepare("SELECT b.platform_member_id,MAX(b.created_at) last_booking_at,COUNT(*) booking_count FROM advisor_bookings b WHERE b.advisor_id=? GROUP BY b.platform_member_id ORDER BY last_booking_at DESC").bind(session.advisor_id).all();return json({customers:rows.results||[]},200,cors)}
  ...
  if(url.pathname==='/api/advisor/earnings'&&request.method==='GET'){const [ledger,statements]=await Promise.all([db.prepare("SELECT * FROM advisor_commission_ledger WHERE advisor_id=? ORDER BY created_at DESC").bind(session.advisor_id).all(),db.prepare("SELECT * FROM advisor_settlement_statements WHERE advisor_id=? ORDER BY period_start DESC").bind(session.advisor_id).all()]);return json({ledger:ledger.results||[],statements:statements.results||[],payout:advisorPaymentReadiness(env)},200,cors)}
  if(url.pathname==='/api/advisor/contracts/current'&&request.method==='GET'){const contract=await db.prepare("SELECT * FROM advisor_contract_versions WHERE is_active=1 ORDER BY created_at DESC LIMIT 1").first(),signature=await db.prepare("SELECT public_id,signed_at,status FROM advisor_contract_signatures WHERE advisor_id=? ORDER BY signed_at DESC LIMIT 1").bind(session.advisor_id).first();return json({contract,signature,legal_gate:contract?.legal_review_status==='approved'?'approved':'LEGAL_REVIEW_REQUIRED'},200,cors)}
...
export async function handleAdvisorAdmin(request,env,url,cors,adminSession){
  ...
  if(url.pathname==='/api/admin/advisor-policies'&&request.method==='GET'){const [policies,commission]=await Promise.all([db.prepare("SELECT * FROM advisor_platform_policies ORDER BY policy_key").all(),db.prepare("SELECT * FROM advisor_commission_policies ORDER BY created_at DESC").all()]);return json({policies:policies.results||[],commission_policies:commission.results||[],payment:advisorPaymentReadiness(env),commission_application_mode:ADVISOR_COMMISSION_MODE},200,cors)}
  ...
  const refund=url.pathname.match(/^\/api\/admin\/advisor-bookings\/([^/]+)\/refund$/);if(refund&&request.method==='POST'){const booking=await db.prepare("SELECT * FROM advisor_bookings WHERE id=?").bind(refund[1]).first();if(!booking)return json({error:'找不到預約。'},404,cors);if(booking.status!=='COMPLETED')return json({error:'只有已完成服務可進入退款流程。'},409,cors);await db.prepare("UPDATE advisor_bookings SET status='REFUND_PENDING',payment_status='REFUND_PENDING',updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(booking.id).run();const reversal=await reverseAdvisorCommission(db,booking.id,clean(request.headers.get('idempotency-key'),160)||`refund:${booking.id}`);return json({status:'REFUND_PENDING',reversal},200,cors)}
