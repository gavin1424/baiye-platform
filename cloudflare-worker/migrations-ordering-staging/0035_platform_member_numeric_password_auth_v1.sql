-- Isolated Staging equivalent of Production migration 0025.
...
CREATE INDEX idx_platform_member_login_credentials_member ON platform_member_login_credentials(platform_member_id,status);

CREATE TABLE platform_member_security_events (
  id TEXT PRIMARY KEY,
  platform_member_id TEXT,
  merchant_id TEXT,
  action TEXT NOT NULL CHECK(action IN('LOGIN_SUCCESS','LOGIN_FAILED','ACCOUNT_LOCKED','PASSWORD_SET','LOGOUT')),
...
CREATE INDEX idx_platform_member_security_events_member ON platform_member_security_events(platform_member_id,created_at);
