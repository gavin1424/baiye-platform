...
INSERT OR REPLACE INTO merchant_users(id,merchant_id,email,password_hash,password_salt,password_iterations,password_algorithm,status,display_name) VALUES('staging_commerce_owner','staging_commerce_merchant','staging-owner@invalid.example',${quote(hash)},${quote(salt)},600000,'pbkdf2-sha256-segmented-v1','active','STAGING Owner');
INSERT OR REPLACE INTO merchant_users(id,merchant_id,email,password_hash,password_salt,password_iterations,password_algorithm,status,display_name) VALUES('staging_commerce_readonly','staging_commerce_merchant','staging-readonly@invalid.example',${quote(hash)},${quote(salt)},600000,'pbkdf2-sha256-segmented-v1','active','STAGING Read Only');
...
INSERT OR IGNORE INTO merchant_roles(id,merchant_id,code,name,is_system) VALUES('staging_commerce_readonly_role','staging_commerce_merchant','read_only','Read Only',1);
...
${readPermissions.map((permission) => `INSERT OR IGNORE INTO merchant_permissions(code,module,description) VALUES(${quote(permission)},${quote(permission.split(".")[0])},${quote(permission)});\nINSERT OR IGNORE INTO merchant_role_permissions(role_id,permission_code) VALUES('staging_commerce_readonly_role',${quote(permission)});`).join("\n")}
...
INSERT OR IGNORE INTO merchant_user_roles(merchant_id,user_id,role_id) VALUES('staging_commerce_merchant','staging_commerce_readonly','staging_commerce_readonly_role');
