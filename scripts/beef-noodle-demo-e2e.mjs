...
const login = await request("/api/merchant-demo/login", { method: "POST", body: JSON.stringify({ username: "baiye-beef-demo", password }) });
assert.equal(login.response.status, 200); const cookie = login.response.headers.get("set-cookie"); assert.ok(cookie); pass("隔離 Demo Credential 建立正式 Merchant Session");
...
assert.equal(paidOrder.body.order.invoice?.status || "NOT_REQUIRED", "PENDING"); assert.equal(paidOrder.body.order.invoice?.invoice_number || null, null); pass("Invoice Request 建立但未啟用 Provider 不產生正式發票號碼");
