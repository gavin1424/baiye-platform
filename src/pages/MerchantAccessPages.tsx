...
export function MerchantRegisterPage() {
  ...
  const [phone, setPhone] = useState(""), [password, setPassword] = useState(""), [consent, setConsent] = useState(false), [loading, setLoading] = useState(false), [result, setResult] = useState<any>(), [notice, setNotice] = useState(""), [seconds, setSeconds] = useState(3);
  useEffect(() => {
    if (!result) return;
    setSeconds(3);
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      const data: any = await merchantOrderingApi("/api/merchant/register", { method: "POST", headers: authHeaders(), body: JSON.stringify({ phone, password, privacy_consent: consent, consent_version: "merchant-registration-v1", ...(intendedPlan ? { intended_plan: intendedPlan } : {}) }) });
  ...
  return <main className="partner-shell merchant-access-shell"><section className="merchant-access-card"><p className="partner-eyebrow">商家帳號免費建立</p><h1>商家免費註冊</h1><p>註冊費用 NT$0。先建立商家帳號與平台會員，註冊成功後再選擇適合的服務方案並確認契約。</p>{intendedPlan && <p className="merchant-intended-plan">已保留您從加入中心選擇的方案意向；最終方案與價格仍由伺服器驗證並由您再次確認。</p>}<form onSubmit={submit}><label>手機號碼<input required type="tel" inputMode="tel" autoComplete="tel" placeholder="09xxxxxxxx" value={phone} onChange={(event) => setPhone(event.target.value)} /></label><label>設定 8 位數字密碼<input required type="password" inputMode="numeric" autoComplete="new-password" pattern="[0-9]{8}" minLength={8} maxLength={8} value={password} onChange={(event) => setPassword(event.target.value.replace(/\D/g, "").slice(0, 8))} /></label><label className="partner-consent"><input required type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} />我已閱讀並同意會員服務、隱私權說明及商家平台相關條款。</label><button className="btn btn-primary btn-lg" disabled={loading}>{loading ? "註冊處理中…" : "免費註冊商家"}</button></form><p className="partner-guidance-note">密碼只用於安全登入；不會因註冊自動簽署或收取方案費用。</p>{notice && <div className="partner-message">{notice}{notice.includes("已有商家") && <><br /><Link to={`/merchant/login${intendedPlan ? `?plan=${encodeURIComponent(intendedPlan)}` : ""}`}>前往商家登入</Link></>}</div>}<Link to={`/merchant/login${intendedPlan ? `?plan=${encodeURIComponent(intendedPlan)}` : ""}`}>已有商家帳號？前往登入</Link></section></main>;
}

export function MerchantLoginPage() {
  const navigate = useNavigate();
  const [search] = useSearchParams(), intendedPlan = search.get("plan") || "";
  const [phone, setPhone] = useState(""), [password, setPassword] = useState(""), [loading, setLoading] = useState(false), [notice, setNotice] = useState("");
  const submit = async (event: React.FormEvent) => { event.preventDefault(); if (loading) return; setLoading(true); setNotice(""); try { const data: any = await merchantOrderingApi("/api/merchant-auth/login", { method: "POST", headers: authHeaders(), body: JSON.stringify({ phone, password }) }); navigate(intendedPlan ? `/merchant/select-plan?plan=${encodeURIComponent(intendedPlan)}` : data.next_url || "/merchant/dashboard", { replace: true }); } catch (error) { setNotice(errorText(error)); } finally { setLoading(false); } };
  return <main className="partner-shell merchant-access-shell"><section className="merchant-access-card"><h1>商家登入</h1>{intendedPlan && <p className="merchant-intended-plan">登入後會返回您原本選擇的方案確認頁。</p>}<form onSubmit={submit}><label>手機號碼<input required type="tel" inputMode="tel" autoComplete="tel" placeholder="09xxxxxxxx" value={phone} onChange={(event) => setPhone(event.target.value)} /></label><label>8 位數字密碼<input required type="password" inputMode="numeric" autoComplete="current-password" pattern="[0-9]{8}" minLength={8} maxLength={8} value={password} onChange={(event) => setPassword(event.target.value.replace(/\D/g, "").slice(0, 8))} /></label><button className="btn btn-primary btn-lg" disabled={loading}>{loading ? "登入處理中…" : "登入商家中心"}</button></form><p className="partner-guidance-note">方案與價格會在登入後由伺服器重新驗證。</p>{notice && <div className="partner-message">{notice}</div>}<Link to={`/merchant/register${intendedPlan ? `?plan=${encodeURIComponent(intendedPlan)}` : ""}`}>尚未註冊？免費註冊商家</Link></section></main>;
}
