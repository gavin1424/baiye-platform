...
export function MerchantRegisterPage() {
  ...
  const submit = async (event: React.FormEvent) => { event.preventDefault(); setLoading(true); setNotice(""); try { await merchantOrderingApi<{ message?: string }>("/api/merchant-auth/register", { method: "POST", body: JSON.stringify({ phone, password, password_confirm: confirm, privacy_consent: privacy, terms_consent: terms }) }); const selected = params.get("plan"); navigate(`/merchant/select-plan${selected ? `?plan=${encodeURIComponent(selected)}` : ""}`, { replace: true }); } catch (error) { setNotice(errorText(error)); } finally { setLoading(false); } };
