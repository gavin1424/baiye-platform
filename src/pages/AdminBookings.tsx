...
export function AdminBookings() {
  ...
  const saveSettings = async (event: React.FormEvent) => {
    ...
    if (!settings) return;
    try {
      await request("/api/admin/booking/settings", {
        method: "PATCH",
        body: JSON.stringify({
          ...settings,
  ...
  const addHour = async (event: React.FormEvent) => {
    event.preventDefault();
    try {
      await request("/api/admin/booking/hours", {
        method: "POST",
        body: JSON.stringify({ ...hour, weekday: Number(hour.weekday) }),
      });
      setMessage("開放時段已新增。");
      await load();
    } catch (error) {
  ...
  const toggleHour = async (item: Hour) => {
    try {
      await request(`/api/admin/booking/hours/${item.id}`, {
        method: "PATCH",
  ...
  const addBlackout = async (event: React.FormEvent) => {
    event.preventDefault();
    try {
      await request("/api/admin/booking/blackouts", {
        method: "POST",
        body: JSON.stringify({
          start_at: new Date(blackout.start_at).toISOString(),
          end_at: new Date(blackout.end_at).toISOString(),
          reason: blackout.reason,
        }),
      });
      setBlackout({ start_at: "", end_at: "", reason: "" });
      setMessage("休假／封鎖時段已新增。");
      await load();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "新增失敗");
    }
  };
  const toggleBlackout = async (item: Blackout) => {
    try {
      await request(`/api/admin/booking/blackouts/${item.id}`, {
        method: "PATCH",
        body: JSON.stringify({ active: !item.active }),
      });
      await load();
  ...
  const saveService = async (item: Service) => {
    try {
      await request(`/api/admin/booking/services/${item.id}`, {
        method: "PATCH",
        body: JSON.stringify({ ...item, active: Boolean(item.active) }),
      });
      setMessage(`${item.name} 已儲存。`);
      await load();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "儲存失敗");
    }
  ...
  const updateStatus = async (code: string, status: string) => {
    try {
      await request(`/api/admin/bookings/${encodeURIComponent(code)}/status`, {
        method: "PATCH",
  ...
  const visibleBookings = useMemo(() => {
    ...
    if (view === "week") start.setDate(now.getDate() - now.getDay());
    if (view === "month") start.setDate(1);
    start.setHours(0, 0, 0, 0);
    const needle = query.trim().toLowerCase();
    return bookings.filter(
      (item) =>
        new Date(item.start_at) >= start &&
    ...
    <main className="finance-shell booking-admin">
      ...
      <header className="finance-hero">
        ...
        <div className="finance-actions">
          <a
            className="booking-admin-link"
            href="https://meilingpatchwork.com/booking/"
            target="_blank"
            rel="noreferrer"
          >
            開啟顧客預約頁
      ...
      {settings && (
        <section className="finance-panel">
          <h2>預約規則</h2>
          <form className="booking-settings-grid" onSubmit={saveSettings}>
            ...
            <label>
              ...
              <input
                ...
                onChange={(e) =>
                  setSettings({
                    ...settings,
                    maximum_advance_days: Number(e.target.value),
                  })
                }
              />
            </label>
            <label>
              ...
              <input
                ...
                onChange={(e) =>
                  setSettings({
                    ...settings,
                    minimum_notice_minutes: Number(e.target.value),
                  })
                }
              />
            </label>
            ...
            <label>
              ...
              <input
                type="number"
                min="0"
                value={settings.cancellation_cutoff_minutes}
                onChange={(e) =>
                  setSettings({
                    ...settings,
                    cancellation_cutoff_minutes: Number(e.target.value),
                  })
                }
              />
            </label>
            <label>
              ...
              <input
                type="number"
                min="0"
                value={settings.reschedule_cutoff_minutes}
                onChange={(e) =>
                  setSettings({
                    ...settings,
                    reschedule_cutoff_minutes: Number(e.target.value),
                  })
                }
              />
            </label>
            ...
            <label className="booking-switch">
              <input
                type="checkbox"
                checked={Boolean(settings.reminders_enabled)}
                onChange={(e) =>
                  setSettings({
                    ...settings,
                    reminders_enabled: e.target.checked ? 1 : 0,
                  })
                }
              />{" "}
              啟用 24 小時 LINE 提醒
            </label>
            <button>儲存預約規則</button>
          </form>
        </section>
      )}
      <section className="finance-panel">
        ...
        <div className="booking-hours-list">
          {hours.map((item) => (
            <button
              ...
              key={item.id}
              onClick={() => void toggleHour(item)}
            >
              {weekdays[item.weekday]} {item.start_time}–{item.end_time}
              <small>{item.active ? "開放中" : "已停用"}</small>
            </button>
          ))}
        </div>
        <form className="booking-hour-form" onSubmit={addHour}>
          <select
            value={hour.weekday}
            onChange={(e) => setHour({ ...hour, weekday: e.target.value })}
          >
          ...
          <input
            ...
            onChange={(e) => setHour({ ...hour, start_time: e.target.value })}
            required
          />
          <span>至</span>
          <input
            type="time"
            value={hour.end_time}
            onChange={(e) => setHour({ ...hour, end_time: e.target.value })}
            required
          />
          <button>新增開放時段</button>
        </form>
      </section>
      <section className="finance-panel">
        <h2>休假／特殊封鎖</h2>
        <div className="booking-hours-list">
          {blackouts.map((item) => (
            <button
              className={item.active ? "active" : ""}
              key={item.id}
              onClick={() => void toggleBlackout(item)}
            >
              {new Date(item.start_at).toLocaleString("zh-TW", {
                timeZone: "Asia/Taipei",
              })}
              –
              {new Date(item.end_at).toLocaleString("zh-TW", {
                timeZone: "Asia/Taipei",
              })}
              <small>
                {item.reason || "未填原因"}・{item.active ? "封鎖中" : "已停用"}
              </small>
            </button>
          ))}
        </div>
        <form className="booking-hour-form" onSubmit={addBlackout}>
          <input
            type="datetime-local"
            value={blackout.start_at}
            onChange={(e) =>
              setBlackout({ ...blackout, start_at: e.target.value })
            }
            required
          />
          <span>至</span>
          <input
            ...
            onChange={(e) =>
              setBlackout({ ...blackout, end_at: e.target.value })
            }
            required
          />
          <input
            ...
            onChange={(e) =>
              setBlackout({ ...blackout, reason: e.target.value })
            }
            placeholder="原因，例如公休"
          />
          <button>新增封鎖時段</button>
        </form>
      </section>
      <section className="finance-panel">
        <h2>服務／課程</h2>
            ...
            <article key={item.id}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setServices((items) =>
                      items.map((v, i) =>
                        i === index
                          ? { ...v, duration_minutes: Number(e.target.value) }
                          : v,
                      ),
                    )
                  }
              ...
              <label>
                ...
                <input
                  type="number"
                  min="0"
                  value={item.buffer_before_minutes}
                  onChange={(e) =>
                    setServices((items) =>
                      items.map((v, i) =>
                        i === index
              ...
              <label>
                ...
                <input
                  type="number"
                  min="0"
                  value={item.buffer_after_minutes}
                  onChange={(e) =>
                    setServices((items) =>
                      items.map((v, i) =>
                        i === index
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setServices((items) =>
                      items.map((v, i) =>
                        i === index ? { ...v, price_text: e.target.value } : v,
                      ),
                    )
                  }
                />
                ...
                <input
                  ...
                  onChange={(e) =>
                    setServices((items) =>
                      items.map((v, i) =>
                        i === index
                          ? { ...v, max_capacity: Number(e.target.value) }
                          : v,
                      ),
                    )
                  }
              ...
              <button onClick={() => void saveService(item)}>儲存服務</button>
            </article>
          ))}
        </div>
      </section>
      <section className="finance-panel">
        ...
        {visibleBookings.length === 0 ? (
          <p className="finance-note">目前沒有符合範圍的預約。</p>
        ) : (
                ...
                <tr key={item.booking_code}>
                  ...
                  <td data-label="操作">
                    <select
                      value={item.status}
                      onChange={(e) =>
                        void updateStatus(item.booking_code, e.target.value)
                      }
                    >
                      {Object.entries(statuses).map(([value, label]) => (
                        <option value={value} key={value}>
