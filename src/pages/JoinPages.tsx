...
export function JoinPage() {
  ...
  return <main className="join-center-shell">
    <header className="join-center-hero"><p className="partner-eyebrow">統一加入／簽約中心</p><h1>加入創百業智慧鏈</h1><p>選擇您的身分或需要的服務方案</p></header>
...
export function MerchantPlanSelectorPage() {
  ...
  const [data, setData] = useState<any>(), [selected, setSelected] = useState(requested), [confirming, setConfirming] = useState(false), [notice, setNotice] = useState(""), [changeMode, setChangeMode] = useState(false);
  useEffect(() => {
  ...
  const assign = async () => {
    ...
    } catch (error: any) { setNotice(errorText(error)); }
  ...
  const requestChange = async () => {
    ...
    try {
      ...
      const result: any = await merchantOrderingApi("/api/merchant/plans/change-requests", { method: "POST", headers: { "idempotency-key": key }, body: JSON.stringify({ plan_id: plan.plan_id }) });
      setNotice(result.code === "PLAN_CHANGE_ALREADY_SUBMITTED" ? "此方案的變更申請已送出，請勿重複提交。" : "申請已送出。原方案會維持不變，待確認費用與生效條件後再通知您。" );
      setData((current: any) => ({ ...current, pending_plan_change: result.request }));
      setChangeMode(false);
    } catch (error) { setNotice(errorText(error)); }
    finally { setConfirming(false); }
  };
  if (!data) return <main className="join-center-shell"><header className="join-center-hero"><h1>選擇商家服務方案</h1><p>{notice || "正在載入您的方案…"}</p></header></main>;
  const description = (item: Plan) => item.plan_id === "baiye_standard_18000_addons" ? "品牌官網、LINE、AI 客服、會員與預約。" : item.plan_id === "baiye_commerce_ai_45000" ? "完整 AI 商城、商品後台、購物車與訂單。" : "QR 點餐、菜單、KDS 與訂單管理。";
  const hasEffectivePlan = Boolean(data.has_effective_plan);
  const selectablePlans: Plan[] = hasEffectivePlan && changeMode ? data.plans.filter((item: Plan) => item.plan_id !== data.active_plan?.plan_id) : data.plans;
  return <main className="join-center-shell merchant-plan-selector"><header className="join-center-hero">{!hasEffectivePlan && <p className="partner-eyebrow">商家服務</p>}<h1>{hasEffectivePlan ? "您的商家方案" : "選擇商家服務方案"}</h1><p>{hasEffectivePlan ? "查看目前方案，或提出方案變更申請。" : "選擇適合的服務方案，再進入契約簽署。"}</p></header>
    {hasEffectivePlan && !changeMode && <section className="merchant-active-plan"><strong>您目前已有有效方案</strong><button className="btn btn-outline" type="button" onClick={() => { setSelected(""); setNotice(""); setChangeMode(true); }}>變更方案</button></section>}
    {hasEffectivePlan && changeMode && <section className="merchant-change-intro"><h2>選擇新方案</h2><p>送出後由專人確認適用費用與生效條件；申請完成前，您目前的方案與既有契約不會變更。</p><button className="btn btn-ghost" type="button" onClick={() => { setChangeMode(false); setSelected(""); }}>返回目前方案</button></section>}
    {(!hasEffectivePlan || changeMode) && <section className="join-plan-grid">{selectablePlans.map((item: Plan) => <article key={item.plan_id} className={`join-plan-card join-selector-card ${selected === item.plan_id ? "is-selected" : ""}`}><span className="join-plan-tag">{item.trial_months ? "前 3 個月系統服務費 NT$0" : item.tagline}</span><h2>{item.name}</h2><p className="join-plan-price">{money(item.price_minor)}{item.plan_id === "baiye_softpos_24000" ? <small>／24 個月</small> : null}</p><p>{description(item)}</p><button className="btn btn-primary" type="button" aria-pressed={selected === item.plan_id} onClick={() => setSelected(item.plan_id)}>{changeMode ? "選擇此方案" : "方案簽署合約"}</button></article>)}</section>}
    {plan && (!hasEffectivePlan || changeMode) && <section className="merchant-plan-confirm"><h2>{changeMode ? "確認變更申請" : "確認方案"}</h2><p><strong>{plan.name}</strong>｜{money(plan.price_minor)}｜{plan.term_months} 個月{plan.trial_months ? `，另有 ${plan.trial_months} 個月試用期（系統服務費 NT$0，不計入正式週期）` : ""}</p>{plan.plan_id === "baiye_softpos_24000" && <p>首次開通費 {money(plan.activation_fee_minor)} ＋ 保證金 {money(plan.deposit_minor)}；第一正式週期 {money(plan.cycle_fee_minor)} 抵扣 {money(plan.first_cycle_credit_minor)} 後尚需 {money(plan.first_cycle_balance_minor)}。</p>}<p>可申請信用卡 24 期零利率；實際分期方式仍依合作銀行／金流服務商審核及提供條件為準。</p>{changeMode && <p>方案變更的差額、退款、補款及生效日期，將依您的現況確認後另行通知；此申請不會立即取代目前方案。</p>}<button className="btn btn-primary btn-lg" type="button" disabled={confirming || Boolean(changeMode && data.pending_plan_change)} onClick={() => void (changeMode ? requestChange() : assign())}>{confirming ? "送出中…" : changeMode ? "送出變更申請" : "確認方案並前往契約"}</button></section>}
