...
export function MerchantPlanSelectorPage() {
  ...
  return <main className="join-center-shell merchant-plan-selector"><header className="join-center-hero"><p className="partner-eyebrow">商家註冊下一步</p><h1>選擇商家服務方案</h1><p>選擇適合的服務方案，再進入正式契約簽署。</p></header>
    ...
    <section className="join-plan-grid">{data.plans.map((item: Plan) => <label key={item.plan_id} className={`join-plan-card join-selector-card ${selected === item.plan_id ? "is-selected" : ""}`}><input type="radio" name="merchant-plan" value={item.plan_id} checked={selected === item.plan_id} onChange={() => setSelected(item.plan_id)} /><span className="join-plan-tag">{item.trial_months ? "前 3 個月免費" : item.tagline}</span><h2>{item.name}</h2><p className="join-plan-price">{money(item.price_minor)}{item.plan_id === "baiye_softpos_24000" ? <small>／24 個月</small> : null}</p><p>{description(item)}</p><strong>方案簽署合約</strong></label>)}</section>
