...
export function MerchantAdminDashboardPage() {
  ...
  return <Shell><section className="merchant-admin-summary"><div><span>店家名稱</span><strong>{data.merchant.name}</strong></div><div><span>管理者</span><strong>{data.administrator.phone_masked || "手機已驗證"}</strong></div><div><span>帳戶狀態</span><strong>{data.account_status}</strong></div><div><span>契約狀態</span><strong>{data.contract.status === "signed" ? "已完成簽署" : "待簽約"}</strong></div><div><span>方案</span><strong>{money(data.plan.discount_price_minor)} 標準方案</strong></div><div><span>服務期間</span><strong>{data.plan.contract_term_months} 個月</strong></div></section>{locked && <section className="merchant-admin-gate"><h2>管理者權限待啟用</h2><p>完成商家平台服務契約與啟用流程後，才可修改商品、預約、會員與營運設定。</p><Link className="btn btn-primary" to="/merchant/contract">完成商家契約</Link></section>}<section className={`merchant-admin-modules ${locked ? "is-locked" : ""}`}>{modules.map(({ icon: Icon, ...item }) => <article key={item.name}><Icon size={30} weight="duotone" /><h2>{item.name}</h2><p>{item.text}</p>{locked && !["契約","商家設定"].includes(item.name) ? <span className="merchant-module-disabled">完成啟用後開放</span> : item.ready ? <Link className="btn btn-outline" to={item.to}>進入管理</Link> : <span>此功能尚未開放</span>}</article>)}</section></Shell>;
}
