...
export function MerchantAdminDashboardPage() {
  ...
  const demoModules = [
    ...
    { icon: LineSegments, name: "LINE 官方帳號", text: "查看 LINE 官方帳號連結狀態", to: "/merchant/line", ready: true },
    { icon: Storefront, name: "商家設定", text: "品牌、介紹、電話、地址與營業時間", to: "/merchant/profile", ready: true },
  ...
  const resetDemo = async () => {
    ...
    if (!window.confirm("再次確認：商品、價格、庫存、訂單、預約、付款與發票操作資料將被重置。")) return;
...
export function MerchantGoogleBookingPage() { return <Shell title="Google 地圖預約"><section className="merchant-admin-state"><h2>Booking URL 已可使用</h2><p>預約網址：baiyeconnect.com/#/booking/beef-noodle-demo</p><p>此功能是 Google 商家資訊導入百工預約頁，不是 Reserve with Google 原生合作 Provider。</p><Link className="btn btn-primary" to="/booking/beef-noodle-demo">開啟預約頁</Link></section></Shell>; }
export function MerchantPaymentsPage() { return <Shell title="付款狀態"><section className="merchant-admin-state"><h2>尚未啟用正式付款</h2><p>只允許櫃檯人工付款紀錄。LINE Pay、Apple Pay、ECPay 與其他正式金流均未啟用。</p><p>目前不進行真實交易。</p></section></Shell>; }
export function MerchantInvoicePage() { return <Shell title="發票狀態"><section className="merchant-admin-state"><h2>正式電子發票 Provider 尚未啟用</h2><p>目前只建立發票申請紀錄，不會產生假發票號碼、財政部開立狀態或 Provider 成功結果。</p></section></Shell>; }
