...
export function MerchantAdminDashboardPage() {
  ...
  return <Shell><section className="merchant-admin-summary"><div><span>店家名稱</span><strong>{data.merchant.name}</strong></div><div><span>身份</span><strong>管理者</strong></div><div><span>示範標記</span><strong>百工官方示範店</strong></div><div><span>會員身份</span><strong>{data.membership?.merchant_relationship ? "已加入百工牛肉麵" : "待建立"}</strong></div><div><span>庫存管理</span><strong>{data.inventory?.total ? `正常 ${data.inventory.normal}｜低庫存 ${data.inventory.low_stock}｜售完 ${data.inventory.sold_out}` : "尚未建立庫存資料"}</strong></div><div><span>交易狀態</span><strong>示範資料／不進行真實交易</strong></div></section>{demo && <section className="merchant-demo-quick"><div><p>百工官方示範</p><h2>百工牛肉麵商家管理中心</h2><span>菜單修改會立即同步；正式付款尚未啟用。</span></div><div className="merchant-demo-quick-actions"><Link className="btn btn-primary" to="/merchant-admin/ordering#ordering-menu"><Package size={20} />修改菜單</Link><Link className="btn btn-outline" to="/merchant/inventory"><Package size={20} />庫存管理</Link><Link className="btn btn-outline" to="/merchant-admin/ordering#ordering-orders"><Receipt size={20} />查看訂單</Link><Link className="btn btn-outline" to="/merchant-admin/ordering/kitchen"><CookingPot size={20} />廚房 KDS</Link><Link className="btn btn-outline" to="/merchant/members"><UsersThree size={20} />查看會員</Link><Link className="btn btn-outline" to="/merchant/bookings"><CalendarCheck size={20} />建立預約</Link><Link className="btn btn-outline" to="/demo/beef-noodle"><Storefront size={20} />查看我的網站</Link><button className="btn btn-ghost danger" onClick={() => void resetDemo()}>重置試用資料</button></div></section>}<section className="merchant-admin-modules">{modules.map(({ icon: Icon, ...item }) => <article key={item.name}><Icon size={30} weight="duotone" /><h2>{item.name}</h2><p>{item.text}</p>{item.ready ? <Link className="btn btn-outline" to={item.to}>進入管理</Link> : <span>此功能尚未開放</span>}</article>)}</section>{notice && <p className="partner-message">{notice}</p>}</Shell>;
}

export function MerchantInventoryPage() {
  const [data, setData] = useState<any>(), [notice, setNotice] = useState(""), [creating, setCreating] = useState(false), [movements, setMovements] = useState<Record<string, any[]>>({});
  const [form, setForm] = useState({ menu_item_id: "", stock_on_hand: "0", low_stock_threshold: "5", inventory_enabled: true, notes: "" });
  const load = () => merchantOrderingApi<any>("/api/merchant-admin/inventory").then(setData).catch((error: any) => { if (error?.status === 401) window.location.hash = "#/merchant/login"; else setNotice(message(error)); });
  useEffect(() => { void load(); }, []);
  const create = async (event: React.FormEvent) => { event.preventDefault(); setNotice(""); try { await merchantOrderingApi("/api/merchant-admin/inventory", { method: "POST", body: JSON.stringify({ ...form, stock_on_hand: Number(form.stock_on_hand), low_stock_threshold: Number(form.low_stock_threshold) }) }); setCreating(false); setForm({ menu_item_id: "", stock_on_hand: "0", low_stock_threshold: "5", inventory_enabled: true, notes: "" }); setNotice("庫存項目已建立。"); await load(); } catch (error) { setNotice(message(error)); } };
  const adjust = async (item: any, action: "restock" | "adjust") => { const raw = window.prompt(action === "restock" ? `目前 ${item.stock_on_hand} 份。請輸入補貨數量（例如 30）` : `目前 ${item.stock_on_hand} 份。請輸入調整數量（例如 -2）`); if (raw == null) return; const quantity = Number(raw); let reason = action === "restock" ? "商家補貨" : window.prompt("請輸入調整理由（例如：報廢）") || ""; try { await merchantOrderingApi(`/api/merchant-admin/inventory/${item.id}/${action}`, { method: "POST", body: JSON.stringify({ adjustment_quantity: quantity, reason }) }); setNotice(action === "restock" ? "補貨完成。" : "庫存調整完成。"); await load(); } catch (error) { setNotice(message(error)); } };
  const toggle = async (item: any) => { try { await merchantOrderingApi(`/api/merchant-admin/inventory/${item.id}`, { method: "PATCH", body: JSON.stringify({ inventory_enabled: !Boolean(item.inventory_enabled) }) }); await load(); } catch (error) { setNotice(message(error)); } };
  const records = async (item: any) => { if (movements[item.id]) { setMovements((current) => { const copy = { ...current }; delete copy[item.id]; return copy; }); return; } try { const result = await merchantOrderingApi<any>(`/api/merchant-admin/inventory/${item.id}/movements`); setMovements((current) => ({ ...current, [item.id]: result.items || [] })); } catch (error) { setNotice(message(error)); } };
  const availableMenu = (data?.menu_items || []).filter((item: any) => !item.inventory_exists);
  if (!data) return <Shell title="庫存管理"><section className="merchant-admin-state"><p>{notice || "正在載入庫存資料…"}</p></section></Shell>;
  return <Shell title="庫存管理"><section className="inventory-toolbar"><div><p>餐點數量庫存</p><strong>{data?.blank ? "目前尚未建立庫存資料" : `正常 ${data?.summary?.normal || 0}｜低庫存 ${data?.summary?.low_stock || 0}｜售完 ${data?.summary?.sold_out || 0}`}</strong></div><button className="btn btn-primary" onClick={() => setCreating(true)}>新增庫存項目</button></section>{creating && <form className="merchant-admin-form inventory-create" onSubmit={create}><h2>新增庫存項目</h2><label>對應餐點<select required value={form.menu_item_id} onChange={(e) => setForm({ ...form, menu_item_id: e.target.value })}><option value="">請選擇餐點</option>{availableMenu.map((item: any) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>目前庫存數量<input required min="0" step="1" inputMode="numeric" value={form.stock_on_hand} onChange={(e) => setForm({ ...form, stock_on_hand: e.target.value })} /></label><label>低庫存提醒數量<input required min="0" step="1" inputMode="numeric" value={form.low_stock_threshold} onChange={(e) => setForm({ ...form, low_stock_threshold: e.target.value })} /></label><label className="inventory-check"><input type="checkbox" checked={form.inventory_enabled} onChange={(e) => setForm({ ...form, inventory_enabled: e.target.checked })} />啟用庫存控制</label><label>備註<textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} /></label><div className="merchant-admin-actions"><button className="btn btn-primary">建立</button><button type="button" className="btn btn-outline" onClick={() => setCreating(false)}>取消</button></div></form>}<section className="inventory-list">{(data?.items || []).map((item: any) => <article key={item.id}><div className="inventory-main"><div><span>商品</span><h2>{item.menu_item_name}</h2></div><div><span>庫存</span><strong>{item.stock_on_hand}</strong></div><div><span>狀態</span><b className={`inventory-status ${item.inventory_status}`}>{item.inventory_status === "sold_out" ? "售完" : item.inventory_status === "low_stock" ? "⚠ 低庫存" : item.inventory_status === "disabled" ? "未啟用控制" : "正常"}</b></div></div><p>低庫存警戒：{item.low_stock_threshold} 份｜最後異動：{item.last_movement_at ? new Date(item.last_movement_at).toLocaleString("zh-TW") : "無"}</p><div className="inventory-actions"><button onClick={() => void adjust(item,"restock")}>補貨</button><button onClick={() => void adjust(item,"adjust")}>調整</button><button onClick={() => void records(item)}>查看紀錄</button><button onClick={() => void toggle(item)}>{item.inventory_enabled ? "停用控制" : "啟用控制"}</button></div>{movements[item.id] && <div className="inventory-ledger">{movements[item.id].map((movement: any) => <div key={movement.id}><strong>{movement.movement_type}</strong><span>{movement.quantity_before} {movement.quantity_delta > 0 ? "+" : ""}{movement.quantity_delta} → {movement.quantity_after}</span><small>{movement.reason || "自動異動"}｜{new Date(movement.created_at).toLocaleString("zh-TW")}</small></div>)}{!movements[item.id].length && <p>尚無異動紀錄。</p>}</div>}</article>)}{data?.blank && !creating && <div className="inventory-empty"><Package size={54} /><h2>目前尚未建立庫存資料</h2><p>餐點仍可正常販售；只有商家自行建立且啟用的項目才會自動扣庫存。</p><button className="btn btn-primary" onClick={() => setCreating(true)}>新增庫存項目</button></div>}</section>{notice && <p className="partner-message">{notice}</p>}</Shell>;
}
...
export function MerchantAccountPage() {
  ...
  return <Shell title="管理者帳戶"><section className="merchant-admin-account">{data && <><dl><dt>身份</dt><dd>管理者</dd><dt>手機</dt><dd>{data.phone_masked}</dd><dt>Platform Member</dt><dd>{data.platform_member?.established ? "已建立" : "未建立"}</dd><dt>百工牛肉麵會員</dt><dd>{data.merchant_membership?.joined ? "已加入" : "未加入"}</dd><dt>商家管理權限</dt><dd>管理者</dd><dt>商家</dt><dd>{data.merchant.name}</dd><dt>帳戶狀態</dt><dd>{data.status === "ACTIVE" ? "啟用" : "待啟用"}</dd></dl><h2>登入裝置／Session</h2><p>目前有效 Session：{data.sessions.length}</p></>}<div className="merchant-admin-actions"><button className="btn btn-outline" onClick={() => void logout(false)}>登出</button><button className="btn btn-outline danger" onClick={() => void logout(true)}>登出所有裝置</button></div>{notice && <p>{notice}</p>}</section></Shell>;
}

export function MerchantGoogleBookingPage() { return <Shell title="Google 地圖預約"><section className="merchant-admin-state"><h2>Booking URL 已可使用</h2><p>示範預約網址：baiyeconnect.com/#/booking/beef-noodle-demo</p><p>此功能是 Google 商家資訊導入百工預約頁，不是 Reserve with Google 原生合作 Provider。</p><Link className="btn btn-primary" to="/booking/beef-noodle-demo">開啟預約頁</Link></section></Shell>; }
export function MerchantPaymentsPage() { return <Shell title="付款狀態"><section className="merchant-admin-state"><h2>尚未啟用正式付款</h2><p>只允許 Manual Counter Payment Demo。LINE Pay、Apple Pay、ECPay 與其他正式金流均未啟用。</p><p>示範資料／不進行真實交易。</p></section></Shell>; }
export function MerchantInvoicePage() { return <Shell title="發票狀態"><section className="merchant-admin-state"><h2>正式電子發票 Provider 尚未啟用</h2><p>可以建立 Invoice Request Demo，但不會產生假發票號碼、MOF issued 狀態或 Provider success。</p></section></Shell>; }
