...
export function MerchantAdminDashboardPage() {
  ...
  return <Shell><section className="merchant-admin-summary"><div><span>店家名稱</span><strong>{data.merchant.name}</strong></div><div><span>身份</span><strong>管理者</strong></div><div><span>會員身份</span><strong>{data.membership?.merchant_relationship ? "已加入百工牛肉麵" : "待建立"}</strong></div><div><span>帳戶狀態</span><strong>{data.account_status}</strong></div><div><span>庫存管理</span><strong>{data.inventory?.total ? `正常 ${data.inventory.normal}｜低庫存 ${data.inventory.low_stock}｜售完 ${data.inventory.sold_out}` : "尚未建立庫存資料"}</strong></div><div><span>契約狀態</span><strong>{demo ? "Demo 試用免簽" : data.contract.status === "signed" ? "已完成簽署" : "待簽約"}</strong></div><div><span>方案</span><strong>{data.plan.plan_name}</strong>{!demo && <small>{money(data.plan.discount_price_minor)} 固定完整方案</small>}</div><div><span>正式 Provider</span><strong>{data.payment_readiness?.production_payment_enabled ? "已就緒" : "尚未啟用正式服務"}</strong></div></section>{demo && <section className="merchant-demo-quick"><div><p>開始試用</p><h2>百工牛肉麵商家管理中心</h2><span>你所做的菜單修改會立即同步到顧客點餐頁。</span></div><div className="merchant-demo-quick-actions"><Link className="btn btn-primary" to="/merchant-admin/ordering#ordering-menu"><Package size={20} />修改菜單</Link><Link className="btn btn-outline" to="/merchant/inventory"><Package size={20} />庫存管理</Link><a className="btn btn-outline" href="#/q/myJghWaqQbCwMInWWsBUf2xRwsR02saT"><Storefront size={20} />查看我的網站</a><a className="btn btn-outline" href="#/q/myJghWaqQbCwMInWWsBUf2xRwsR02saT"><QrCode size={20} />建立測試訂單</a><Link className="btn btn-outline" to="/merchant-admin/ordering/kitchen"><CookingPot size={20} />開啟 KDS</Link><Link className="btn btn-outline" to="/merchant/members"><UsersThree size={20} />會員</Link><button className="btn btn-ghost danger" onClick={() => void resetDemo()}>重置試用</button></div></section>}{locked && <section className="merchant-admin-gate"><h2>管理者權限待啟用</h2><p>完成商家平台服務契約與啟用流程後，才可修改商品、預約、會員與營運設定。</p><Link className="btn btn-primary" to="/merchant/contract">完成商家契約</Link></section>}<section className={`merchant-admin-modules ${locked ? "is-locked" : ""}`}>{modules.map(({ icon: Icon, ...item }) => <article key={item.name}><Icon size={30} weight="duotone" /><h2>{item.name}</h2><p>{item.text}</p>{locked && !["契約","商家設定"].includes(item.name) ? <span className="merchant-module-disabled">完成啟用後開放</span> : item.ready ? <Link className="btn btn-outline" to={item.to}>進入管理</Link> : <span>此功能尚未開放</span>}</article>)}</section>{notice && <p className="partner-message">{notice}</p>}</Shell>;
}
