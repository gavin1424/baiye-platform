import { CheckCircle, Clock, Globe, ShieldCheck, ShoppingCart, UsersThree } from "@phosphor-icons/react";
...
function PlanContractCta({ plan }: { plan: CommercialPlan }) {
  const navigate = useNavigate();
  const [checking, setChecking] = useState(false);
  const contractPath = planContractPath(plan.plan_slug);
  const openContract = async (event: React.MouseEvent<HTMLAnchorElement>) => {
  ...
  return <Link className="btn btn-primary btn-lg" to={contractPath} onClick={(event) => void openContract(event)} aria-disabled={checking}>{checking ? "正在確認登入…" : "方案簽署合約"}</Link>;
}
...
function PlanCard({ plan }: { plan: CommercialPlan }) {
  ...
  return <PremiumCard className={`pricing-v2-card pricing-v2-${tone}`}>
    <div className="pricing-v2-card-head"><h2>{plan.display_name}</h2><p>{plan.summary}</p><div className="pricing-v2-amount"><strong>{formatTwd(plan.price_minor)}</strong><span>／{plan.term_months} 個月</span></div>{plan.trial_months > 0 && <p className="pricing-v2-trial"><Clock weight="fill" />前 {plan.trial_months} 個月系統服務費免費</p>}</div>
    <div className="pricing-v2-contract-actions"><PlanContractCta plan={plan} /></div>
  </PremiumCard>;
}

const comparisonRows = [
  ["網站", "品牌網站", "完整商城", "點餐入口"], ["AI 客服", "包含", "包含", "可依需求整合"], ["LINE", "包含", "可整合", "選配"], ["會員", "包含", "包含", "包含"], ["預約", "包含", "可整合", "可整合"], ["網站預約", "包含", "可整合", "可整合"], ["商品管理", "百工協助 20 項", "商家完整管理", "商家菜單管理"], ["商家自行修改", "不開放完整 CMS", "完整商品後台", "菜單與營運資料"], ["購物車", "加購", "包含", "點餐購物車"], ["訂單", "加購", "包含", "包含"], ["QR 點餐", "加購", "可整合", "包含"], ["庫存", "加購／依需求", "可整合", "包含"], ["出餐看板", "加購／依需求", "可整合", "包含"], ["標準金流串接能力", "加購", "包含建置能力", "依 readiness"], ["服務期間", "24 個月", "24 個月", "前 3 月免費＋24 個月"], ["加購方式", "附件 B／補充協議", "非標準需求另行確認", "第三方與設備另計"],
] as const;
...
export function PricingPageV13() {
  ...
  return <PublicLayout>
    ...
    <MarketingSection className="pricing-v2-terms"><div className="pricing-trust-row">{[[ShieldCheck, "價格由伺服器方案目錄提供"], [CheckCircle, "電子契約完整保存"], [UsersThree, "真人協助導入"], [Globe, "第三方能力依 readiness"]].map(([Icon, label]) => { const I = Icon as typeof ShieldCheck; return <span key={String(label)}><I weight="duotone" />{String(label)}</span>; })}</div><div className="softpos-disclosure"><ShoppingCart weight="duotone" /><p><strong>SoftPOS 商業條件：</strong>首次開通費 NT$3,000；保證金 NT$6,000 可抵第一個 24 個月週期，尚需 NT$18,000；後續每 24 個月 NT$24,000（平均 NT$1,000／月）。{catalog?.installment_disclosure}</p></div></MarketingSection>
...
export function TermsPageV13() {
  const sections = [["服務性質", "創百業智慧鏈提供商家數位升級、平台上架、品牌網站、商城、智慧點餐、內容刊登與營運工具。"], ["方案與商業條件", "公開價格由伺服器權威方案目錄提供；實際交易仍以雙方簽署之有效商家服務契約、商業條件 Snapshot 及補充協議為準。"], ["第三方服務", "金流、分期、電子發票、LINE、簡訊、物流與其他 Provider，僅於完成正式資格、憑證與正式環境驗證後啟用。"], ["額外需求", "原方案外新增頁面、特殊設計、第三方 API、客製流程或人工內容修改，須另行評估、報價與確認。"], ["付款、退款與契約優先", "付款、退款、修改、交付、驗收、智慧財產、資料使用與終止條件，以雙方實際簽署之有效契約為準。"]];
  return <PublicLayout><MarketingHero eyebrow="平台規範" title="使用條款與商家商業方案" description="公開資訊協助比較；具有法律效果的權利義務，以雙方正式簽署的契約版本為準。" /><MarketingSection className="legal-page-v2"><article>{sections.map(([h, t], i) => <section id={`v13-${i}`} key={h}><span>{String(i + 1).padStart(2, "0")}</span><div><h2>{h}</h2><p>{t}</p></div></section>)}</article></MarketingSection></PublicLayout>;
}
