...
export function PricingPageV13() {
  return <PublicLayout>
    ...
    <div className="pricing-trust-row">{[[ShieldCheck,"一般方案不按交易金額抽成"],[CheckCircle,"標準網站免費附贈"],[UsersThree,"真人客服支援"],[Globe,"超出標準規格另行報價"]].map(([Icon,label])=>{const I=Icon as typeof ShieldCheck;return <span key={String(label)}><I weight="duotone" />{String(label)}</span>})}</div>
    <section className="pricing-faq section"><SectionHeading title="訂金代收與銷售抵付是選配服務" /><div className="faq-grid"><article><h3>一般 upfront_18000 方案</h3><p>商家已一次支付 NT$18,000，不再建立 NT$18,000 銷售抵付進度，避免重複收費。一般商家方案不按交易金額抽成。</p></article><article><h3>sales_offset_18000 選配方案</h3><p>只有另行申請訂金代收與月結對帳服務、通過審核且有效契約載明者，才以每期平台作業服務費逐期抵付；完成後是否繼續計費依契約設定。</p></article></div><Link className="text-link" to="/services/deposit-settlement">查看訂金代收與月結對帳說明</Link></section>
...
export function FaqPageV13() {
  const items = [
    ...
    ["平台會按每筆交易抽成嗎？","一般商家方案不按交易金額抽成；另行申請訂金代收與月結對帳服務者，依個別契約及實際金流成本計費。已一次支付 NT$18,000 的商家不會再建立銷售抵付進度。"],
...
export function TermsPageV13() {
  const sections = [
    ...
    ["訂金代收與月結對帳選配服務","一般商家方案不按交易金額抽成。另行申請訂金代收與月結對帳服務者，實際費率、抵付、代收、撥款、退款、稅務預留及扣繳，以雙方有效契約、正式對帳單及適用法令為準；相關條款於完成律師與記帳士審閱前不視為正式定稿。"],
  ];
