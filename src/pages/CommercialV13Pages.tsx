...
export function PricingPageV13() {
  ...
  return <PublicLayout>
    ...
    <MarketingSection className="pricing-v2-terms"><div className="pricing-trust-row">{[[ShieldCheck, "方案費用清楚列示"], [CheckCircle, "簽署前完整確認"], [UsersThree, "真人協助導入"], [Globe, "第三方服務依合作條件提供"]].map(([Icon, label]) => { const I = Icon as typeof ShieldCheck; return <span key={String(label)}><I weight="duotone" />{String(label)}</span>; })}</div><div className="softpos-disclosure"><ShoppingCart weight="duotone" /><p><strong>免 POS 機智慧點餐商業條件：</strong>前三個月系統服務費 NT$0；首次開通費 NT$3,000；保證金 NT$6,000 可抵第一個 24 個月週期，尚需 NT$18,000；後續每 24 個月 NT$24,000（平均 NT$1,000／月）。{catalog?.installment_disclosure}</p></div></MarketingSection>
...
export function HowItWorksPageV13() {
  return <PublicLayout><MarketingHero eyebrow="如何運作" title="從免費帳號，到簽約與正式營運" description="先確認商家需求，再選擇 18k 標準、45k 完整商城或 24k 免 POS 點餐方案；每一步都有清楚的契約與啟用狀態。" primary={{ label: "免費建立商家帳號", to: "/merchant/register" }} secondary={{ label: "比較三種方案", to: "/pricing" }} /><MarketingSection className="how-v2"><SectionHeading eyebrow="導入流程" title="五個步驟，讓權限與服務範圍不混淆" /><div className="path-steps">{[["1", "免費建立商家帳號", "使用手機與 8 位數字密碼建立商家申請。"], ["2", "選擇適合方案", "依品牌網站、完整商城或智慧點餐需求選擇方案。"], ["3", "確認契約與商業條件", "價格、期間、試用、保證金與加購內容會在簽署前完整顯示。"], ["4", "完成簽約與啟用", "完成電子簽署及必要的付款確認後，開啟對應服務。"], ["5", "進入商家管理中心", "依方案管理商品、訂單、會員、預約、庫存或出餐流程。"]].map(([n, t, d]) => <PremiumCard className="card-stagger" key={n}><span className="path-number">{n}</span><h2>{t}</h2><p>{d}</p></PremiumCard>)}</div></MarketingSection><CTASection title="先比較方案，再決定導入節奏" description="不需要把所有功能一次裝上；我們會依商家現況確認最合適的服務範圍。" primary={{ label: "查看商家方案", to: "/pricing" }} secondary={{ label: "聯絡平台", to: "/contact" }} /></PublicLayout>;
}

export function FaqPageV13() {
  const items = [["目前有哪些商家方案？", "共有百工標準方案 NT$18,000／24 個月、AI 智慧商城完整版 NT$45,000，以及免 POS 機智慧點餐 NT$24,000／24 個月。"], ["18k 標準方案可以自己改完整網站嗎？", "網站主要內容與基礎 20 項商品／服務由百工協助建立與維護。超出範圍以加購報價和補充協議辦理。"], ["45k 完整商城包含什麼？", "固定完整方案包含 AI、完整商城、商品管理後台、購物車、訂單管理、商家管理者與標準金流串接能力；實際付款方式須依合作金流服務商審核及提供條件開通。"], ["免 POS 點餐的前三個月與保證金怎麼算？", "前 3 個月系統服務費 NT$0；首次開通費 NT$3,000、保證金 NT$6,000。保證金可抵第一個 24 個月週期，因此首週期尚需 NT$18,000；後續每 24 個月 NT$24,000。"], ["24 期零利率一定可以使用嗎？", "不一定。實際分期方案依合作銀行／金流服務商核准及當時可用條件為準。"], ["標準方案加購如何計價？", "加購項目會顯示參考價格；標示「起」的項目須由百工依實際需求確認，簽署前會完整列出最終費用。"], ["第三年續用費如何計算？", "18k 標準方案如沿用目前契約條件，第 3 年起選擇續用時為平台上架 NT$3,000、網域 NT$1,000、後台／網站維持 NT$3,000，合計 NT$7,000／年。"]];
