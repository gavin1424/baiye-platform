...
const planTone: Record<string, string> = { baiye_standard_18000_addons: "standard", baiye_commerce_ai_50000: "commerce", baiye_softpos_24000: "softpos" };
...
function PlanCard({ plan }: { plan: CommercialPlan }) {
  ...
  return <PremiumCard className={`pricing-v2-card pricing-v2-${tone}`}>
    ...
    <Link className="btn btn-primary btn-lg" to={`/merchant/select-plan?plan=${plan.plan_id}`}>方案簽署合約</Link>
    ...
    {tone === "commerce" && <p className="pricing-v2-note">商家可自行管理商品、價格、圖片、分類、規格與上下架；付款方式依合作金流服務商審核及實際提供條件為準。</p>}
    {tone === "softpos" && <div className="pricing-v2-fees"><span>簽約首期款 <strong>NT$6,000</strong></span><span>三個月試用期後尾款 <strong>NT$18,000</strong></span><span>契約總額 <strong>NT$24,000</strong></span></div>}
  </PremiumCard>;
}

const comparisonRows = [
  ["網站", "品牌網站", "完整商城", "點餐入口"], ["AI 客服", "包含", "包含", "可依需求整合"], ["LINE", "包含", "可整合", "選配"], ["會員", "包含", "包含", "包含"], ["預約", "包含", "可整合", "可整合"], ["Google 地圖預約", "包含", "可整合", "可整合"], ["商品管理", "百工協助 20 項", "商家完整管理", "商家菜單管理"], ["商家自行修改", "由百工協助維護", "完整商品後台", "菜單與營運資料"], ["購物車", "加購", "包含", "點餐購物車"], ["訂單", "加購", "包含", "包含"], ["QR 點餐", "加購", "可整合", "包含"], ["庫存", "加購／依需求", "可整合", "包含"], ["出餐看板", "加購／依需求", "可整合", "包含"], ["標準金流串接能力", "加購", "包含建置能力", "依合作條件提供"], ["服務期間", "24 個月", "24 個月", "前 3 月試用＋24 個月"], ["加購方式", "附件 B／補充協議", "非標準需求另行確認", "第三方與設備另計"],
] as const;

export function PricingPageV13() {
  const { catalog, error } = useCatalog(); const plans = catalog?.plans || [];
  return <PublicLayout>
    <MarketingHero eyebrow="商家數位升級方案" title="從品牌官網，到完整商城與智慧點餐" description="依商家營運需求，選擇適合的數位升級方案。方案金額與期間以頁面顯示及簽署文件為準。" primary={{ label: "免費建立商家帳號", to: "/merchant/register" }} secondary={{ label: "比較方案", to: "/pricing?compare=1" }} className="pricing-v2-hero"><div className="pricing-hero-chips">{plans.length ? plans.map((plan) => <span key={plan.plan_id}><strong>{formatTwd(plan.price_minor)}{plan.plan_id.includes("softpos") ? "／24個月" : ""}</strong><small>{plan.short_name}</small></span>) : <span><strong>3 種商家方案</strong><small>正在載入方案</small></span>}</div></MarketingHero>
    ...
    <MarketingSection className="pricing-v2-plans" id="plans"><SectionHeading eyebrow="三種商家方案" title="選擇符合現在營運階段的方案" description="免費商家帳號是進入方案流程的起點，不與付費方案混列；簽署前會再次顯示完整方案與費用。" />{error && <div className="catalog-error" role="alert">{error}</div>}<div className="pricing-v2-grid">{plans.map((plan, index) => <div id={`plan-${planTone[plan.plan_id]}`} className="card-stagger" style={{ "--stagger": `${index * 80}ms` } as CSSProperties} key={plan.plan_id}><PlanCard plan={plan} /></div>)}</div>{!plans.length && !error && <div className="catalog-loading">正在讀取方案…</div>}</MarketingSection>
    <MarketingSection className="pricing-v2-comparison" id="comparison"><SectionHeading eyebrow="方案比較" title="功能、權限與服務期間，一次看清楚" description="桌面使用完整表格；手機可逐項閱讀，不需要左右拖曳超寬表格。" /><div className="pricing-comparison-desktop" role="table" aria-label="三種商家方案比較"><div className="pricing-comparison-row pricing-comparison-head" role="row"><strong>比較項目</strong><span>18k 標準</span><span>50k 商城</span><span>24k 點餐</span></div>{comparisonRows.map((row) => <div className="pricing-comparison-row" role="row" key={row[0]}><strong>{row[0]}</strong>{row.slice(1).map((value) => <span key={value}>{value}</span>)}</div>)}</div><div className="pricing-comparison-mobile">{comparisonRows.map((row) => <PremiumCard key={row[0]}><h3>{row[0]}</h3><dl><dt>18k 標準</dt><dd>{row[1]}</dd><dt>50k 商城</dt><dd>{row[2]}</dd><dt>24k 點餐</dt><dd>{row[3]}</dd></dl></PremiumCard>)}</div></MarketingSection>
    {catalog && <MarketingSection className="pricing-v2-addons"><SectionHeading eyebrow="標準方案加購服務" title="超出 20 項或特殊功能，以加購方式處理" description="標示「起」的項目須由百工依實際需求確認最終報價；簽署前會完整列出費用。" /><div className="addon-grid">{catalog.standard_addons.map((addon, index) => <PremiumCard className="card-stagger" key={addon.code}><span>{String(index + 1).padStart(2, "0")}</span><h3>{addon.label}</h3><strong>{addon.display_price}</strong>{addon.admin_quote_required && <small>須人工確認最終報價</small>}</PremiumCard>)}</div></MarketingSection>}
    <MarketingSection className="pricing-v2-terms"><div className="pricing-trust-row">{[[ShieldCheck, "方案費用清楚列示"], [CheckCircle, "簽署前完整確認"], [UsersThree, "真人協助導入"], [Globe, "第三方服務依合作條件提供"]].map(([Icon, label]) => { const I = Icon as typeof ShieldCheck; return <span key={String(label)}><I weight="duotone" />{String(label)}</span>; })}</div><div className="softpos-disclosure"><ShoppingCart weight="duotone" /><p><strong>免 POS 機智慧點餐商業條件：</strong>契約總額 NT$24,000；簽約首期款 NT$6,000；自服務啟用日起三個月為試用期，試用期結束後支付尾款 NT$18,000。</p></div></MarketingSection>
...
export function MerchantPlanSelectPage() {
  ...
  return <PublicLayout>
    <MarketingHero eyebrow="選擇商家方案" title="選擇符合營運需求的方案" description="選擇後會再次顯示方案內容與適用價格供您確認，再進入契約簽署。" secondary={{ label: "返回方案比較", to: "/pricing" }} />
...
export function HowItWorksPageV13() {
  return <PublicLayout><MarketingHero eyebrow="如何運作" title="從免費帳號，到簽約與正式營運" description="先確認商家需求，再選擇 18k 標準、50k 完整商城或 24k 免 POS 點餐方案；每一步都有清楚的契約、付款與啟用狀態。" primary={{ label: "免費建立商家帳號", to: "/merchant/register" }} secondary={{ label: "比較三種方案", to: "/pricing" }} /><MarketingSection className="how-v2"><SectionHeading eyebrow="導入流程" title="五個步驟，讓權限與服務範圍不混淆" /><div className="path-steps">{[["1", "免費建立商家帳號", "使用手機與 8 位數字密碼建立商家申請。"], ["2", "選擇適合方案", "依品牌網站、完整商城或智慧點餐需求選擇方案。"], ["3", "確認契約與商業條件", "價格、期間、試用與付款排程會在簽署前完整顯示。"], ["4", "完成簽約與付款", "電子簽署後完成付款回報，經平台確認入帳才正式啟用。"], ["5", "進入商家管理中心", "依方案管理商品、訂單、會員、預約、庫存或出餐流程。"]].map(([n, t, d]) => <PremiumCard className="card-stagger" key={n}><span className="path-number">{n}</span><h2>{t}</h2><p>{d}</p></PremiumCard>)}</div></MarketingSection><CTASection title="先比較方案，再決定導入節奏" description="不需要把所有功能一次裝上；我們會依商家現況確認最合適的服務範圍。" primary={{ label: "查看商家方案", to: "/pricing" }} secondary={{ label: "聯絡平台", to: "/contact" }} /></PublicLayout>;
}

export function FaqPageV13() {
  const items = [["目前有哪些商家方案？", "共有百工標準方案 NT$18,000、AI 智慧商城完整版 NT$50,000，以及契約總額 NT$24,000 的免 POS 機智慧點餐方案。"], ["18k 標準方案如何付款？", "簽約時一次支付 NT$18,000，平台確認入帳後契約生效。"], ["50k 完整商城如何付款？", "簽約時一次支付 NT$50,000，平台確認入帳後契約生效。"], ["免 POS 點餐如何付款？", "契約總額 NT$24,000；簽約首期款 NT$6,000，服務啟用日起三個月試用期結束後支付尾款 NT$18,000。"], ["付款回報後會立即啟用嗎？", "付款截圖與回報不等同已確認入帳；須由平台管理員核對後，契約才生效並啟用服務。"], ["標準方案加購如何計價？", "加購項目會顯示參考價格；標示「起」的項目須由百工依實際需求確認，簽署前會完整列出最終費用。"]];
  return <PublicLayout><MarketingHero eyebrow="常見問題" title="三種商家方案，關鍵條件一次說清楚" description="價格、內容權限、智慧點餐試用期、付款排程、加購與第三方服務限制，都以清楚白話呈現。" primary={{ label: "比較方案", to: "/pricing" }} secondary={{ label: "聯絡我們", to: "/contact" }} /><MarketingSection><div className="faq-grid faq-v2">{items.map(([q, a]) => <PremiumCard key={q}><h3>{q}</h3><p>{a}</p></PremiumCard>)}</div></MarketingSection></PublicLayout>;
}

export function TermsPageV13() {
  const sections = [["服務性質", "創百業智慧鏈提供商家數位升級、平台上架、品牌網站、商城、智慧點餐、內容刊登與營運工具。"], ["方案與商業條件", "公開頁面提供方案價格與條件；實際交易仍以雙方簽署之有效商家服務契約、商業條件及補充協議為準。"], ["第三方服務", "金流、分期、電子發票、LINE、簡訊、物流與其他第三方服務，僅於完成必要資格、憑證與使用條件後啟用。"], ["額外需求", "原方案外新增頁面、特殊設計、第三方系統串接、客製流程或人工內容修改，須另行評估、報價與確認。"], ["付款、退款與契約優先", "付款、退款、修改、交付、驗收、智慧財產、資料使用與終止條件，以雙方實際簽署之有效契約為準。"]];
