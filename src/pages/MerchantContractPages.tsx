...
export function MerchantContractActivate() {
  ...
  return <main className="partner-shell contract-shell"><h1>啟用商家 Owner 帳號</h1>{invite && <section className="contract-summary-card"><strong>{invite.merchant_name}</strong><span>{invite.email}</span><span>{invite.plan_name} · {money(invite.discount_price_minor)}</span></section>}<form className="partner-form" onSubmit={submit}><label>簽署人顯示名稱<input required value={form.display_name} onChange={(event) => setForm({ ...form, display_name: event.target.value })} /></label><label>設定 8 位數字密碼<input required type="password" inputMode="numeric" pattern="[0-9]{8}" minLength={8} maxLength={8} autoComplete="new-password" value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value.replace(/\D/g, "").slice(0, 8) })} /></label><label>再次確認 8 位數字密碼<input required type="password" inputMode="numeric" pattern="[0-9]{8}" minLength={8} maxLength={8} autoComplete="new-password" value={form.password_confirm} onChange={(event) => setForm({ ...form, password_confirm: event.target.value.replace(/\D/g, "").slice(0, 8) })} /></label><button className="btn btn-primary">建立帳號</button></form>{notice && <div className="partner-message">{notice}</div>}<Link to="/merchant/contract">前往商家契約</Link></main>;
}
