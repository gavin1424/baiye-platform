...
export function MerchantContractActivate() {
  ...
  return <main className="partner-shell contract-shell"><h1>商家註冊</h1>{invite && <section className="contract-summary-card"><strong>{invite.merchant_name}</strong><span>{invite.plan_name} · {money(invite.discount_price_minor)}</span></section>}{invite && !success && <form className="partner-form" onSubmit={submit}><label>手機號碼<input required type="tel" inputMode="tel" placeholder="09xxxxxxxx" value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} /></label><label className="partner-consent"><input required type="checkbox" checked={form.privacy_consent} onChange={(event) => setForm({ ...form, privacy_consent: event.target.checked })} />我已閱讀並同意會員服務、隱私權說明及商家平台相關條款。</label><button className="btn btn-primary">完成商家註冊</button><p className="partner-guidance-note">不用設定密碼，使用手機即可註冊與登入。</p></form>}{success && <section className="partner-status success"><strong>🎉 商家註冊成功！</strong><span>✓ 商家帳號已建立</span><span>✓ 創百業會員已建立</span><span>✓ 會員經營功能已連結</span><span><strong>下一步：完成商家平台服務契約</strong></span><span>{seconds} 秒後自動前往商家契約</span><Link className="btn btn-primary" to="/merchant/contract">立即前往簽約</Link></section>}{notice && <div className="partner-message">{notice}</div>}<Link to="/merchant">前往商家中心</Link></main>;
}

const requiredConsentKeys = ["read", "commercial_terms", "authority", "signature_evidence", "electronic"] as const;
...
export function MerchantContractPage() {
  ...
  const validateBeforePreview = () => {
    ...
    if (!requiredConsentKeys.every((key) => form[key])) return "請完成全部 5 項契約確認。";
  ...
  const decideRenewal = async (continueService: boolean) => {
    setNotice("");
    try {
      const result: any = await merchantOrderingApi(continueService ? "/api/merchant/contracts/renewal/prepare" : "/api/merchant/contracts/renewal/decline", { method: "POST", body: JSON.stringify({ continue_service: continueService }) });
      setNotice(continueService ? `第 ${result.cycle.cycle_number} 週期待付金額：${money(result.cycle.balance_due_minor)}。${result.payment_provider.disclosure}` : `已選擇不續用。${result.data_retention}`);
      await load();
    } catch (error) { setNotice(message(error)); }
  };
  ...
  if (context.signed) return <main className="partner-shell contract-shell"><h1>商家平台服務契約已完成簽署</h1><p>版本 {context.contract.version} · {context.signature.signed_at}</p>{context.renewal && <section className="contract-summary-card"><strong>{context.renewal.subscription.renewal_state === "RENEWAL_REQUIRED" ? "是否續用免 POS 機智慧點餐系統" : `服務狀態：${context.renewal.subscription.renewal_state}`}</strong><span>免費試用：{context.renewal.subscription.trial_started_at} ～ {context.renewal.subscription.trial_ends_at}</span>{context.renewal.subscription.renewal_state === "RENEWAL_REQUIRED" && <><span>第一週期尚應支付 {money(context.plan.first_cycle_balance)}；不續用將停止正式服務功能並依契約保留資料。</span><div className="partner-workflow-actions"><button className="btn btn-primary" type="button" onClick={() => void decideRenewal(true)}>續用，建立第一週期</button><button className="btn btn-outline" type="button" onClick={() => void decideRenewal(false)}>不續用</button></div></>}</section>}<div className="partner-workflow-actions"><button className="btn btn-primary" onClick={() => void downloadMerchantContractPdf(context.signature.id, context.signature.public_id).catch((error) => setNotice(message(error)))}>下載契約檔案</button><Link className="btn btn-outline" to="/merchant">返回商家中心</Link></div>{notice && <div className="partner-message">{notice}</div>}{memberWelcome && <div className="contract-confirm-dialog member-welcome-modal" role="dialog" aria-modal="true"><div><div className="member-celebration">🎉</div><h2>{memberWelcome.title}</h2><p>您的創百業會員資格也已建立。</p><div className="partner-workflow-actions"><Link className="btn btn-primary" to="/member">前往會員中心</Link><Link className="btn btn-outline" to="/merchant">返回商家中心</Link></div></div></div>}</main>;

  const consentLabels: Record<(typeof requiredConsentKeys)[number], string> = context.plan ? {
    read: "我已完整閱讀本契約及附件 A。",
    commercial_terms: "我確認開通費 NT$3,000、保證金 NT$6,000，第一週期抵充後尚付 NT$18,000。",
    authority: "我確認免費試用 3 個月，正式計價為 NT$24,000／24 個月，非逐月短約。",
    signature_evidence: "我了解 24 期零利率仍須依實際金融／支付機構核准與 Provider 可用能力為準。",
    electronic: "我同意使用電子形式完成契約簽署並保存 Evidence、PDF、Hash 與 Audit。",
  } : {
    read: "我已完整閱讀本契約及附件 A。",
  ...
  return <main className="partner-shell contract-shell"><p className="partner-eyebrow">線上契約簽署</p><h1>{context.contract.title}</h1>{context.staging && <p className="contract-staging-notice">STAGING｜法律審閱測試版本</p>}{context.plan ? <><section className="contract-summary-grid"><article><span>開通費</span><strong>{money(context.plan.activation_fee)}</strong></article><article><span>保證金</span><strong>{money(context.plan.deposit)}</strong></article><article><span>前三個月</span><strong>免費</strong></article><article><span>正式方案</span><strong>{money(context.plan.cycle_fee)}／24 個月</strong></article><article><span>第一週期抵充後</span><strong>尚需 {money(context.plan.first_cycle_balance)}</strong></article><article><span>後續週期</span><strong>{money(context.plan.renewal_fee)}／24 個月</strong></article></section><section className="contract-summary-card"><strong>免專用 POS 主機</strong><span>不等於完全零硬體；仍需商家自備營運所需裝置與網路。</span><span>{context.plan.payment_provider.disclosure}</span><span>Provider 實際 24 期能力：{context.plan.payment_provider.ready ? "已驗證" : "尚未驗證，不會產生假交易"}</span></section></> : <section className="contract-summary-grid"><article><span>商家</span><strong>{context.merchant.name}</strong></article><article><span>方案</span><strong>{context.terms.plan_name}</strong></article><article><span>總價</span><strong>{money(context.terms.discount_price_minor)}</strong></article><article><span>付款方式</span><strong>{context.terms.payment_plan === "upfront_18000" ? "一次付清" : context.terms.payment_plan}</strong></article><article><span>服務期間</span><strong>{context.terms.contract_term_months} 個月</strong><span>{context.terms.start_date} ～ {context.terms.service_period_end}</span></article><article><span>契約版本</span><strong>{context.contract.version}</strong></article></section>}<section className="contract-summary-card"><strong>契約雙方</strong>{legalEntityMissing ? <span>甲方資料設定尚未完成，簽署功能暫時鎖定。</span> : <><span>甲方：{context.legal_entity.entity.legal_name}（統編：{context.legal_entity.entity.tax_id}）</span><span>乙方：{context.merchant.name}</span></>}</section><article className="contract-document" aria-label="商家平台服務契約正文" dangerouslySetInnerHTML={{ __html: context.contract.content_html }} />{(context.attachments || []).map((attachment: any) => <article className="contract-document contract-attachment" key={attachment.title} dangerouslySetInnerHTML={{ __html: attachment.contentHtml || attachment.content || "" }} />)}{legalEntityMissing ? <section className="partner-status warning"><strong>平台法律主體設定尚未完成</strong><span>缺少：{(context.legal_entity?.missing_fields || []).map((item: any) => item.label).join("、") || "必要資料"}。Staging 不會填入假公司資料。</span></section> : <><div className="contract-form-grid"><label>簽署人法定姓名<input required value={form.signatory_legal_name} onChange={(event) => setForm({ ...form, signatory_legal_name: event.target.value })} /></label><label>簽署身份<select value={form.signatory_role} onChange={(event) => setForm({ ...form, signatory_role: event.target.value })}><option value="legal_representative">法定代表人</option><option value="authorized_representative">受授權代表</option></select></label><label>法定代表人姓名<input required value={form.legal_representative_name} onChange={(event) => setForm({ ...form, legal_representative_name: event.target.value })} /></label><label>統一編號（如適用）<input value={form.tax_id} onChange={(event) => setForm({ ...form, tax_id: event.target.value })} /></label></div>{form.signatory_role === "authorized_representative" && <label className="partner-consent"><input type="checkbox" checked={form.authorization_confirmed} onChange={(event) => setForm({ ...form, authorization_confirmed: event.target.checked })} />本人確認已取得代表商家簽署本契約之合法授權。</label>}{requiredConsentKeys.map((key) => <label key={key} className="partner-consent"><input type="checkbox" checked={form[key]} onChange={(event) => setForm({ ...form, [key]: event.target.checked })} />{consentLabels[key]}</label>)}<section className="merchant-signature-notice"><strong>本人正楷手寫簽名</strong><p>請以正楷清楚簽寫本人完整姓名，簽署姓名須與上方填寫之簽署人姓名一致。</p><p>請正楷簽寫：{form.signatory_legal_name || "您的完整姓名"}</p></section><ContractSignatureCanvas onChange={setSignature} minimumStrokes={2} minimumPoints={12} clearLabel="清除重寫" /><button className="btn btn-primary" type="button" onClick={() => void previewSign()} disabled={isSigning}>預覽最後確認</button></>}{preview && <div className="contract-confirm-dialog" role="dialog" aria-modal="true"><div><h2>簽署前最後確認</h2><dl><dt>商家名稱</dt><dd>{preview.company_name}</dd><dt>契約版本</dt><dd>{preview.version}</dd><dt>方案</dt><dd>{preview.plan_name}</dd><dt>{preview.plan ? "第一週期尚付" : "總價"}</dt><dd>{money(preview.total_minor)}</dd><dt>服務期間</dt><dd>{preview.term_months} 個月（{preview.period.start} ～ {preview.period.end}）</dd><dt>付款條件</dt><dd>{preview.plan ? preview.plan.payment_provider.disclosure : (preview.payment_plan === "upfront_18000" ? "一次付清" : preview.payment_plan)}</dd><dt>簽署人</dt><dd>{preview.signatory}</dd><dt>簽署身份</dt><dd>{roleLabel(preview.signatory_role)}</dd></dl><p>確認後將建立不可變 PDF 與私人 Evidence JSON。手寫軌跡屬線上簽署證據，不是憑證式數位簽章。</p><div className="partner-workflow-actions"><button className="btn btn-outline" disabled={isSigning} onClick={() => setPreview(undefined)}>返回修改</button><button className="btn btn-primary" disabled={isSigning} onClick={() => void sign()}>{isSigning ? "簽署處理中…" : "確認簽署"}</button></div></div></div>}{notice && <div className="partner-message">{notice}</div>}</main>;
}
