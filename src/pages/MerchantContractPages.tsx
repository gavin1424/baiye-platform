...
const DEFAULT_STANDARD_TOTAL_LABEL = "NT$18,000";
const requiredConsents = (context?: any) => [
  ["read", "我已完整閱讀本契約及附件 A。"],
  ["commercial_terms", `我確認本方案總價為 ${context?.terms ? money(context.terms.discount_price_minor) : DEFAULT_STANDARD_TOTAL_LABEL}。`],
  ["authority", `我確認服務期間為 ${context?.terms?.contract_term_months || 24} 個月。`],
...
export function MerchantContractPage() {
  ...
  const validateBeforePreview = () => {
    ...
    if (!requiredConsents(context).every(([key]) => form[key])) return "請完成全部 5 項契約確認。";
