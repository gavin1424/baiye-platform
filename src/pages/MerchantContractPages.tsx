...
export function MerchantContractPage() {
  ...
  if (context.signed) return <main className="partner-shell contract-shell"><h1>商家平台服務契約已簽署</h1><p>版本 {context.contract.version} · {context.signature.signed_at}</p><Link className="btn btn-primary" to="/merchant/contracts">查看與下載歷史契約</Link>{memberWelcome && <div className="contract-confirm-dialog member-welcome-modal" role="dialog" aria-modal="true"><div><div className="member-celebration">🎉</div><h2>{memberWelcome.title}</h2><p>您的創百業會員資格也已建立，NT$100 迎新禮券已放入帳戶。</p><div className="partner-workflow-actions"><Link className="btn btn-primary" to="/member">查看我的優惠券</Link><Link className="btn btn-outline" to="/merchant/contracts">繼續前往商家契約中心</Link></div></div></div>}</main>;
