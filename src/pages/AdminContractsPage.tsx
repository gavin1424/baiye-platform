...
export function AdminContractsPage() {
  ...
  const assignCommercePlan = async () => {
    try {
      const result = await adminApi(`/api/admin/merchants/${commerceMerchantId}/commerce-ai-45000-plan`, { method: "POST", headers: { "idempotency-key": crypto.randomUUID() }, body: JSON.stringify({ plan_id: "baiye_commerce_ai_45000", confirm_fixed_price: true }) });
      setInvite({ ...invite, merchant_id: commerceMerchantId, commercial_terms_id: result.commercial_terms_id });
      setNotice(`已指派 AI 智慧商城完整版固定方案 NT$45,000：${result.assignment_id}`);
    } catch (error) { setNotice(errorText(error)); }
  };
