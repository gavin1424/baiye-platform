...
class MerchantApi(
    ...
    private fun parseJob(j: JSONObject): PrintJob = PrintJob(j.getString("id"), j.optString("merchant_id"), j.optString("order_id"), j.getString("order_code"), j.getString("printer_id"), j.optString("status"), j.optInt("copies", 1), j.optInt("attempt_count"), j.optJSONObject("payload")?.toString() ?: "{}", deliveryOutcome = j.optString("delivery_outcome"), lastError = j.optString("last_error"), isReprint = j.optBoolean("is_reprint"))
