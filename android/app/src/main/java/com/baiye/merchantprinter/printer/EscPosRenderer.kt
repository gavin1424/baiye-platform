...
class EscPosRenderer(private val mode: ChineseRenderMode = ChineseRenderMode.BITMAP) {
    ...
    private fun kitchenLines(payload: JSONObject): List<Line> = buildList {
            ...
            add(Line("付款狀態：${if (payload.optString("payment_status") == "paid") "已付款" else "未付款"}", 30f))
            add(Line("總額：NT$ ${payload.optInt("total_minor") / 100}", 42f, bold = true))
            add(Line("接單時間：${payload.optString("accepted_at").takeIf { it.isNotBlank() }?.let(::formatTime) ?: "待接單"}", 28f))
            add(Line("================================", 25f))
    ...
    fun testReceipt(now: Date = Date()): ByteArray {
        return raster(listOf(
            ...
            Line("免pos機智慧點餐", 42f, Paint.Align.CENTER, true),
